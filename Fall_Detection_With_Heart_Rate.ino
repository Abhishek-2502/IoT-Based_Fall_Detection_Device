...
void setup(){
  ...
  {
    delay(500);
    Serial.print("WiFi connecting");              // print connecting till not connected
    Serial.println("");
  }
  Serial.println("");
...
void loop(){
  ...
  if (Amp>=17 && trigger2==false){ //if AM breaks lower threshold (0.4g)   ///////
    trigger1=true;
    Serial.println("TRIGGER 1 ACTIVATED");
  }
  if (trigger1==true){
    trigger1count++;
    if (Amp>=22){ //if AM breaks upper threshold (3g)    //////
      trigger2=true;
...
void send_event(const char *event){
  ...
  url += privateKey;
  url += "?value1="; 
  url += String(pox.getHeartRate()); 
  url += "&value2="; 
  url += String(pox.getSpO2()); 
